...
class Type11Menu: ItemView {
    ...
    @IBOutlet private weak var itemValueContainerView: UIView!
    ...
    override var valueFontColor: UIColor {
        didSet {
            ...
            self.itemQuantity.textColor = valueFontColor
            self.separatorView.backgroundColor = valueFontColor
        }
    }
...
extension Type11Menu {
    private func addDataInContainer() {
        ...
        if let values = data?.values, values.count > 0 {
            ...
            self.itemValueContainerView.isHidden = true
        }
        self.layoutIfNeeded()
    }
}

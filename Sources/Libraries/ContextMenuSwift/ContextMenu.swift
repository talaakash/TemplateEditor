...
public struct ContextMenuItemWithImage: ContextMenuItem {
    ...
    public init(title: String, image: UIImage, isEnabled: Bool) {
        self.title = title
        self.image = image
...
extension ContextMenu : UITableViewDataSource, UITableViewDelegate {
    ...
    open func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if self.delegate?.contextMenuDidSelect(forRowAt: indexPath.row) ?? false {
            self.closeAllViews()
        }
    }
    
    open func tableView(_ tableView: UITableView, didDeselectRowAt indexPath: IndexPath) {
        self.delegate?.contextMenuDidDeselect(forRowAt: indexPath.row)
    }

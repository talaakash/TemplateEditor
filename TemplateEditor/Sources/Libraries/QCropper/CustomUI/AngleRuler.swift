...
public class AngleRuler: UIControl {
    ...
    let bordersWidth: CGFloat = 1.0 / UIScreen.main.scale
    // Make pixel alignment to avoid anti-aliasing, 0.5 = lineWidth / 2
    private lazy var pixelOffset = CGFloat(0.5.truncatingRemainder(dividingBy: Double(self.bordersWidth)))
    ...
    private lazy var midScaleLineBorder: UIView = {
        let view = UIView(frame: CGRect(x: 0, y: 0, width: 1 + 2 * bordersWidth, height: 30 + 2 * bordersWidth))
    ...
    private func setupScaleLayers() {
        ...
        scaleBorders.lineWidth = 1 + 2 * bordersWidth
        ...
        for i in 0 ... numberOfTotalScales {
            ...
            borderPath.move(to: CGPoint(x: x, y: lineTop - bordersWidth))
            borderPath.addLine(to: CGPoint(x: x, y: lineBottom + bordersWidth))
        }
